...
# Метод, що генерує масив необхідної довжини в заданому діапазоні
def generate_array(length, min_val, max_val):
    return [random.randint(min_val, max_val) for _ in range(length)]


# Метод, що виводить елементи масиву у вказаному форматі
def print_array(arr):
    formatted_elements = [f"[елемент_{i + 1}_значення_{val}]" for i, val in enumerate(arr)]
    print(",\n".join(formatted_elements))


# 1. Порахувати кількість та суму парних елементів масиву, що знаходяться в заданому діапазоні
...
# 2. Визначити середнє арифметичне елементів масиву та кількість елементів, що більші за нього
...
# 3. Утворити третій масив як попарну суму елементів двох масивів однакової довжини
def pairwise_sum(arr1, arr2):
    return [a + b for a, b in zip(arr1, arr2)]


# 4. Утворити третій масив як конкатенацію двох масивів різної довжини
def concatenate_arrays(arr1, arr2):
    return arr1 + arr2


# 5. В масиві поміняти місцями максимум та мінімум
...
# 6. Масив поділити на два масиви: з додатніх та від’ємних елементів
...
def remove_duplicates_min_max(arr):
    ...
    min_val = min(arr)
    max_val = max(arr)
    # Залишаємо елементи, які не є дублікатами мінімума чи максимума
    # (або залишаємо по одному екземпляру, залежно від умов, тут видаляємо всі входження min/max окрім перших, або повністю екстремуми)
    # Зробимо класичне очищення дублікатів екстремумів, залишивши унікальні або видаливши повтори:
    filtered = []
    min_seen = False
    max_seen = False
    for val in arr:
...
# 8. Визначити середні арифметичні двох масивів і третій масив з елементів між ними
